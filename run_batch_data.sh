#!/usr/bin/env bash
# Batch-run run_video_inference.py on every .mp4 under INPUT_DIR (recursive).
# Output preserves the source's subdirectory layout under OUT_ROOT.
# Skips any scene whose output dir already has a complete run.

set -euo pipefail

INPUT_DIR="${INPUT_DIR:-/home/builder/workspace/data}"
OUT_ROOT="${OUT_ROOT:-/home/builder/workspace/lingbot-map/outputs/data}"
...
mapfile -t VIDEOS < <(find "$INPUT_DIR" -type f -name '*.mp4' | sort)
...
for video in "${VIDEOS[@]}"; do
    ...
    rel="${video#$INPUT_DIR/}"
    rel_noext="${rel%.mp4}"
    out_dir="$OUT_ROOT/$rel_noext"
    log="$out_dir/run.log"

    echo "" | tee -a "$BATCH_LOG"
    echo "[$idx/${#VIDEOS[@]}] $rel_noext" | tee -a "$BATCH_LOG"
